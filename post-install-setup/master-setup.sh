...
echo "Validate :: Ingress Controller Type: LoadBalancer External IP: 192.168.210.200"
#echo "192.168.210.200	nginx.example.com" | sudo tee -a /etc/hosts
...
tee dashboard-devsecops-ingress.yaml <<EOF
...
kubectl apply -f dashboard-devsecops-ingress.yaml

# Test Nginx full deployment
tee nginx-complete.yaml <<EOF
apiVersion: apps/v1
kind: Deployment
metadata:
  name: nginx
...
spec:
  ingressClassName: nginx
  rules:
  - host: test.nginx.192.168.210.200.nip.io
    http:
      paths:
      - backend:
          service:
  ...
  tls:
  - hosts:
    - test.nginx.192.168.210.200.nip.io
