...
echo "Removing all swap entries from fstab"
sudo sed -i '/ swap /d' /etc/fstab  # Removes all swap entries, not just swapfile

echo "Checking fstab to ensure swap is removed"
grep swap /etc/fstab  # Should return nothing if swap is removed

echo "Disabling swap permanently"
sudo systemctl mask swap.target  # Prevent swap from being enabled on boot
